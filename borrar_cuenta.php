        ...
        <form method="post">
            ...
            if (isset($_POST['borrar'])) {
                ...
                if ($rowA['password'] == $passActual) {
                    ...
                    $sqlD = $mysqli->query("SELECT * FROM users WHERE id  = '" . $_SESSION['id'] . "'");
                    $rowD = $sqlD->fetch_array();
                    echo "entro el if de update";


                 $deletecreator = $mysqli->query("DELETE FROM creador WHERE id_creador = '" . $rowC['id'] . "' ");
                    $deletefollow = $mysqli->query("DELETE FROM seguidores WHERE seguido = '" . $rowD['id'] . "'");
                    $deletepublication = $mysqli->query("DELETE FROM publicaciones WHERE user = '" . $rowD['id'] . "'");
                    $deletesuscription = $mysqli->query("DELETE FROM pago WHERE pagador = '" . $rowD['id'] . "'");
                    $deletemessage = $mysqli->query("DELETE FROM mensajes WHERE de = '" . $rowD['id'] . "'");
                    $deletelikes = $mysqli->query("DELETE FROM likes WHERE usuario = '" . $rowD['id'] . "'");
                    $deletearchive = $mysqli->query("DELETE FROM archivo WHERE user = '" . $rowD['id'] . "'");
                    $deletecount = $mysqli->query("DELETE FROM users WHERE id = '" . $rowD['id'] . "'");

                    if ($deletecount) {
                        header("refresh: 0; url = logout.php");
                    }
            ?>
            <p style="color:white"> <?php echo "Tu cuenta ha sido borrada exitosamente, favor de cerrar sesion"; ?> </p>
            <?php
            ...
            <div class="form" id="sarch-box">
                ...
                <label for="email" class="form__label">Introduce tu contraseña</label>
            </div>
            ...
            <button name="borrar" class="btn btn--2" id="inptsearch">
                Borrar Cuenta
            </button>
        </form>
    </div>
